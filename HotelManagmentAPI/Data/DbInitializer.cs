    ...
    public static class DbInitializer
    {
        public static async Task InitAsync(IApplicationBuilder appBuilder)
        {
            using (var serviceScope = appBuilder.ApplicationServices.CreateScope())
            {
                ...
                {
                    await SeedData(dbContext, GetSampleHotels());
                }
                if (!await dbContext.Rooms.AnyAsync())
                {
                    await SeedData(dbContext, GetSampleRooms());
                }
                if (!await dbContext.Roles.AnyAsync())
                {
                    await SeedData(dbContext, GetSampleRoles());
                }
            }
        }

        private static async Task SeedData<T>(ApplicationDbContext db, IEnumerable<T> entities) where T : class
        {
            await db.Set<T>().AddRangeAsync(entities);
            await db.SaveChangesAsync();
        }

        private static IEnumerable<Hotel> GetSampleHotels()
        {
        ...
        private static IEnumerable<Room> GetSampleRooms()
        {
        ...
        private static IEnumerable<IdentityRole> GetSampleRoles()
        {
